...
**Статус:** `Containerfile` (раздел 6) — подтверждённо рабочая **сборка**. `entrypoint.sh` (раздел 7) — подтверждён рабочим на уровне запуска контейнера, инициализации кластера, реального TDS-подключения **и выполнения T-SQL-запросов**: Azure Data Studio успешно подключается на порт 1433 и выполняет `SELECT` без ошибок. Это подтверждает разом несколько вещей:
...
- `babelfishpg_tsql.database_name`/`migration_mode`, прописанные в `postgresql.conf` до первого старта, и убранный ошибочный `GRANT ALL ON SCHEMA dbo` — вся цепочка инициализации отрабатывает до состояния, в котором TDS-клиент реально подключается и выполняет запросы;
- сам движок Babelfish (парсер T-SQL, транслятор `SELECT` в план PostgreSQL) реально работает end-to-end, не только на уровне логина.

**Не проверено:** `SELECT` — это read-only чтение; операции записи (`INSERT`/`UPDATE`/`CREATE TABLE`) в `dbo`-схеме, другие клиенты (`tsql`/`sqlcmd` — используют другой клиентский стек, чем ADS), и T-SQL обёртки PostGIS (`master_dbo.postgis_version()` и т.д.) — не были в фокусе этой проверки.
...
3. **Операции записи (`INSERT`/`CREATE TABLE`) и другие клиенты (`tsql`/`sqlcmd`) — следующий шаг проверки.** `SELECT` через ADS подтверждён; запись данных и другие клиентские стеки (FreeTDS-based `tsql`, `sqlcmd`) — ещё нет. Особенно важно для приложений, которые будут подключаться не через ADS.
4. **T-SQL обёртки PostGIS в `master_dbo`** (раздел 21.1) — не проверены в реальном сеансе. Иллюстративный пример на две функции, не полное покрытие API. Расширяйте по необходимости под свои задачи.
