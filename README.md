...
**Статус:** `Containerfile` (раздел 6) — подтверждённо рабочая **сборка**. `entrypoint.sh` (раздел 7) — подтверждён рабочим на уровне запуска контейнера, инициализации кластера **и реального TDS-подключения**: Azure Data Studio успешно подключается на порт 1433 без ошибок. Это подтверждает разом несколько вещей:

- `password_encryption = 'md5'` действительно решает несовместимость TDS-протокола со SCRAM — TDS-аутентификация реально работает;
- заглушки `xp_msver`/логика `master_dbo` не мешают подключению (ADS их либо использует корректно, либо не запрашивает вовсе — в любом случае ошибок нет);
- `babelfishpg_tsql.database_name`/`migration_mode`, прописанные в `postgresql.conf` до первого старта, и убранный ошибочный `GRANT ALL ON SCHEMA dbo` — вся цепочка инициализации отрабатывает до состояния, в котором TDS-клиент реально может достучаться до сервера.

**Не проверено:** сама проверка была именно про факт подключения (ADS вошёл без ошибок), а не про выполнение T-SQL-запросов. Открытыми остаются: реальные запросы (`SELECT`/`CREATE TABLE` и т.п.) через ADS, подключение через `tsql`/`sqlcmd` (используют другой клиентский стек, чем ADS, — не гарантированно идентичное поведение), и T-SQL обёртки PostGIS (`master_dbo.postgis_version()` и т.д.) — они не были в фокусе этой проверки.
...
3. **Реальные T-SQL-запросы и другие клиенты (`tsql`/`sqlcmd`) — следующий шаг проверки.** Подключение через ADS подтверждено; выполнение запросов и другие клиентские стеки (FreeTDS-based `tsql`, `sqlcmd`) — ещё нет. Особенно важно для приложений, которые будут подключаться не через ADS.
4. **T-SQL обёртки PostGIS в `master_dbo`** (раздел 21.1) — не проверены в реальном сеансе. Иллюстративный пример на две функции, не полное покрытие API.
5. **`babelfishpg_tsql.database_name`/`migration_mode` в `postgresql.conf` до старта, а не через `ALTER SYSTEM`.** Логика правдоподобна и итоговая связка подтверждённо работает, но я не смог независимо на 100% подтвердить именно этот конкретный механизм (что без переноса в `postgresql.conf` подключение не работало бы) — возможно, оба подхода были бы равнозначны.
6. **Суффикс `-5` в символических ссылках `babelfishpg_tsql-5.so`/`babelfishpg_common-5.so` (раздел 6).** Подобран под `BABEL_5_4_0` по аналогии с номером релиза — не подтверждён как официально документированное правило именования Babelfish. Контейнер с этими симлинками успешно проходил инициализацию и принимал TDS-подключения — работают как минимум не хуже, чем без них. При смене версии Babelfish перепроверяйте по факту ошибки `could not access file`.
